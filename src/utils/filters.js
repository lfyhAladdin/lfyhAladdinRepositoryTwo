...
const moneyFormat = (value, type = true, unit) => {
  ...
  if (!isNaN(val)) {
    ...
    if(unit){
      ...
      let s1 = (val/unit).toString().split(".");
      let decimal1 = s1[1] && s1[1].length < 2 ? s1[1] + '0' : s1[1];
      // return val/unit + suffix
      return s1[0].replace(/(\d)(?=(?:\d{3})+$)/g, '$1,') + "." + (decimal1 || '00') + suffix
    }
...
const telFormat = (value) => {
  let tel = value.substring(0, 3) + ' ' + value.substring(3, 7) + ' ' + value.substring(7, 11);
  return tel;
};
// 格式化电话号码 中间4位替换成*
...
// 身份证号 中间替换成*
const cardIDNoHideFormat = (value) => {
  if (value.length > 8) {
