...
export default class Request {
  ...
  // private 通过interceptor.request回调方法赋值
   requestBeforeFun (config) {
    return config
  }

  // private 通过interceptor.response成功回调赋值
  requestComFun (response) {
    return response
  }

  // private 通过interceptor.response失败回调赋值
  ...
  async request (options = {}) {
    ...
    return new  Promise( async(resolve, reject) => {
      ...
      options.complete = (response) => {
        response.config = handleRe // 对象引用，具体赋值在后面 handleRe = { ...this.requestBeforeFun(options, cancel) }
      ...
      let beforeConfig = await this.requestBeforeFun(options,cancel)
      handleRe = { ...beforeConfig }
  ...
  upload (url, {
    ...
    return new Promise((resolve, reject) => {
      ...
      let beforeConfig=  this.requestBeforeFun(pubConfig, cancel)
      handleRe = { ...beforeConfig  }
      const _config = { ...handleRe }
      if (!next) return
      delete _config.custom
