<template>
    ...
    <view :class="touchmove ? 'uni-indexed-list__menu--active' : ''" class="uni-indexed-list__menu">
			<view v-for="(list, key) in lists" :key="key" class="uni-indexed-list__menu-item" @click="menuItemClick(key)">
				<text class="uni-indexed-list__menu-text">{{ list.key }}</text>
			</view>
		</view>
    <!-- #endif -->
	...
	export default {
		...
		methods: {
			setList() {
				...
				yu.createSelectorQuery()
					.in(this)
					.select('#list')
					.boundingClientRect()
					.exec(ret => {
						this.winOffsetY = ret[0].top
						this.winHeight = ret[0].height - (this.immergeBottom || 0)
