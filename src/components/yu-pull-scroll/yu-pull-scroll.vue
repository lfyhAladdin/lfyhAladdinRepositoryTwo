<template>
  <view :class="['s-pull-scroll',customClass]" @touchmove.stop.prevent="e=>e.preventDefault()">
    <scroll-view :id="scrollId" class="s-pull-scroll-view" :class="{'is-fixed':fixed}" :style="{'padding-top':padTop,'padding-bottom':padBottom,'top':fixedTop,'bottom':fixedBottom}" :scroll-top="scrollTop" :scroll-with-animation="false" :scroll-y="scrollAble" :enable-back-to-top="true" @scroll="scroll" @touchstart="touchstart" @touchmove="touchmove" @touchend="touchend" @touchcancel="touchend">
      <view :style="{'transform': translateY, 'transition': transition}" :class="contentClass">
...
export default {
  ...
  computed: {
    ...
    fixedTop () {
      // 沉浸式状态栏时，scroll距离顶部的数据应该是
      // "手机状态栏高度+用户传入的距离页面顶部的高度"
    ...
    translateY () {
      return this.downHight > 0 ? (this.downHight > this.numDownMaxDis ? 'translateY(' + this.numDownMaxDis + 'px)' : 'translateY(' + this.downHight + 'px)') : ''; // transform会使fixed失效,需注意把fixed元素写在mescroll之外
    },
    // 下拉刷新的最大限度数值,单位rpx,需转成px.
    numDownMaxDis () {
      return (Array.isArray(this.downMaxDis) ? this.downMaxDis : [this.downMaxDis]).map(num => uni.upx2px(Number(num || 0))).reduce((a, b) => a + b) || 0;
    }
	},
  ...
  created () {
    ...
    uni.getSystemInfo({
      success: (res) => {
        console.log('滚动条', JSON.stringify(res));
        // if (res.windowTop) this.windowTop = res.windowTop; // 修正app和H5的top值
        this.windowTop = res.statusBarHeight || 0; // 修正app和H5的top值
  ...
  methods: {
    ...
    touchmove (e) {
      ...
      if (moveY > 0 && (scrollRealTop <= 0 || (scrollRealTop <= this.numDownStartTop && scrollRealTop === this.startTop))) {
        // 可下拉的条件
        if (!this.inTouchend && !this.isDownLoading && !this.isUpLoading) {
          // 下拉的角度是否在配置的范围内
    ...
    triggerPullUp (isCheck) {
      if (!this.isUpLoading && !this.isDownLoading && !this.isError && !this.isFinish) {
        // 是否校验在底部; 默认不校验
