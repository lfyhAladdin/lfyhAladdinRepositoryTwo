...
<script>
	export default {
		...
		methods: {
			showDrawer() {
				yu.getSubNVueById('drawer').show('slide-in-left', 200);
			},
		    showPopup() {
				// 向 popup 传递消息
				yu.$emit('page-popup', {
					title: '请阅读软件内容',
					content: 'yu-app 是一个使用 Vue.js 开发跨平台应用的前端框架，开发者编写一套代码，可编译到iOS、Android、H5、小程序等多个平台。'
				});
				const subNVue = yu.getSubNVueById('popup')
			...
			playVideo() {
				let subNVue = yu.getSubNVueById('video_mask')
