...
export default {
	...
	methods: {
		...
		startBluetoothDevicesDiscovery() {
			yu.startBluetoothDevicesDiscovery({
				success: e => {
		...
		stopBluetoothDevicesDiscovery(types) {
			yu.stopBluetoothDevicesDiscovery({
				success: e => {
		...
		onBluetoothDeviceFound() {
			yu.onBluetoothDeviceFound(devices => {
		...
		getBLEDeviceCharacteristics() {
			...
			yu.getBLEDeviceCharacteristics({
				// 这里的 deviceId 需要已经通过 createBLEConnection 与对应设备建立链接
				deviceId,
		...
		onBLEConnectionStateChange() {
			yu.onBLEConnectionStateChange(res => {
		...
		onBLECharacteristicValueChange() {
			// 必须在这里的回调才能获取
			yu.onBLECharacteristicValueChange(characteristic => {
		...
		/**
		 * 订阅操作成功后需要设备主动更新特征值的 value，才会触发 yu.onBLECharacteristicValueChange 回调。
		 */
		notifyBLECharacteristicValueChange() {
			...
			console.log(characteristicId);
			console.log(notify);
			yu.notifyBLECharacteristicValueChange({
