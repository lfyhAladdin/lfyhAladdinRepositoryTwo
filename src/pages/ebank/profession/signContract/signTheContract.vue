<template>
	<view class="uni-container">
		...
		<pfSearch :switchDrawer="switchDrawer" :applyPhase="applyPhase" :serviceStatesType="tabTitle" :pfSearchBusiness="pfSearchBusiness"
		 @professionSearch="professionSearch" @changeSearch="changeSearch"></pfSearch>
		...
		<view class="pSubmit-cover" v-if="coverAll">
			<view class="cover-submit">
				...
				<view class="contract-ul">
					...
					<view class="contract-li">
						<text>确认房屋预售合同号</text>
						<input placeholder="再次输入房屋预售合同号" type="text" id="houseContractID" class="iptcontract" v-model="houseContractTwo"
						 onpaste="return false" oncut="return false" @blur="confirmInput" />
					</view>
				</view>
	...
	export default {
		...
		methods: {
			...
			querypriceresult(e) {
				...
				if (
					this.businessTypeInfo == "1140010" ||
					this.businessTypeInfo == "1140110"
				) {
					this.coverAll = true;
				} else {
			...
			toPerfectInformation(e) {
				...
				if (this.houseData.status != "待签署") {
					...
					let createdata = {
						applyNo: this.houseData.applyNo, //申请编号（必输）
						contractNo: this.contractNo, //合同编号,
						status: "3", //3-合同待生效 4-合同已生效 
					};
					this.interfaceRequest(
						"/api/aldapploanprocessinfo/create",
						createdata,
						"post",
						(res) => {
							console.log(res, "添加贷款流程信息状态");
