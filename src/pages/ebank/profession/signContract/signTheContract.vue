...
<script>
	...
	export default {
		...
		methods: {
			...
			querypriceresult(e) {
				...
				if (this.businessTypeInfo == "个人一手住房贷款" || this.businessTypeInfo == "个人一手商用房贷款") {
					this.coverAll = true;
				} else {
    ...
    setTimeout(() => {
     this.isDisable = false
				...
				if (res.businessType == "个人二手住房贷款" || res.businessType == "个人二手商用房贷款") {
					let data = {
						orderNo: "",
								...
								yu.showModal({
									...
									success: res => {
										if (res.confirm) {
											...
											this.querypriceapprapplyinfo(this.beginNoData, this.totalNoData);
											this.queryApplyPhaseList();
										}
									}
								});
...
this.isDisable = true
 ...
 if (regStr.test(this.certificateRegistration)) {
    this.certificateRegistration = this.certificateRegistration.replace(regStr, "");
 }
			},
