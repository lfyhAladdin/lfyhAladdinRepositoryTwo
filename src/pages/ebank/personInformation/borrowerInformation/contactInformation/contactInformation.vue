    ...
    <view class="customize-content-form" :key="timer">
      ...
      <view v-for="(item,index) in customerConcatList" :key="index+'a'">
        ...
        <view class="person-infor">
          ...
          <view class="person-infor-one">
            <text>与借款人关系</text>
            <picker @change="relationShipChange($event,index)" :value="relationShipIdxList[index]" :range-key="'value'" :range="relationShipList">
    ...
    export default {
        ...
        methods: {
            ...
            relationShipChange(e, idx) {
                ...
                this.customerConcatList[idx].relationShip = this.relationShipList[e.target.value].key;
                this.$forceUpdate();
            },
            // 获取与借款人关系列表
            getRelationShipList() {
                    ...
                    if (resData.returnCode == 'Success') {
                        if (resData.customerConcatList != null) {
                            ...
                            that.customerConcatList.forEach((item, index) => {
                                that.relationShipIdxList.push(that.selectInfo(that.relationShipList, item.relationShip));
                            });
                        }else{
                            console.log("无联系人数据")
